...
def assign_criticality_and_upload(file_path: str, result: dict, criticality_config: dict) -> dict:
    ...
    # Add storage_type and retention_period to result for database storage
    result['storage_type'] = storage_type
    result['retention_period'] = retention_period
...
async def dashboard(request: Request):
    ...
    # Redirect admin users to admin dashboard (with GhostLayer stats and DB monitoring)
    if user.get("role") == "admin":
        logger.info("Admin user, redirecting to admin dashboard")
...
@app.get("/api/admin/ghostlayer-stats")
async def get_admin_ghostlayer_stats(request: Request):
    """Get comprehensive GhostLayer statistics for admin dashboard"""
    try:
        # Verify admin access
        user_data = require_auth(request)
        ...
        stats = db.get_user_ghostlayer_stats()
        return stats
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error fetching admin GhostLayer stats: {e}")
        raise HTTPException(status_code=500, detail="Failed to fetch statistics")

@app.get("/api/admin/top-users-ai-classification")
async def get_top_users_ai_classification(request: Request):
    """Get top users by AI Document Classification uploads for admin dashboard"""
    try:
        # Verify admin access
        user_data = require_auth(request)
        if not user_data or user_data.get('role') != 'admin':
        ...
        # Get top users from database using existing db instance
        import sqlite3
        import os
        ...
        logger.info(f"Active users count: {user_count}")
        
        cursor.execute("""
            SELECT 
                u.username,
                COUNT(adc.id) as upload_count,
                COALESCE(SUM(adc.file_size), 0) as total_storage,
                MAX(adc.upload_timestamp) as last_upload
            FROM users u
            LEFT JOIN ai_document_classifications adc ON u.id = adc.user_id
            WHERE u.is_active = 1
            GROUP BY u.id, u.username
            ORDER BY upload_count DESC, u.username ASC
            LIMIT 10
        """)
        
        users = []
        for row in cursor.fetchall():
            # Only include users who have actual uploads
            if row[1] > 0:  # upload_count > 0
                users.append({
                    'username': row[0],
                    'upload_count': row[1],
        ...
        conn.close()
        logger.info(f"Returning {len(users)} users with uploads")
        return users
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error fetching top users AI classification: {e}")
        logger.error(f"Database path: {db.db_path}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Failed to fetch top users data: {str(e)}")

@app.get("/api/ai-documents")
async def get_user_ai_documents(request: Request, page: int = 1, limit: int = 10):
    """Get user's AI document classifications (admin sees all documents)"""
    try:
        ...
        if user_role == 'admin':
            # Get ALL AI document classifications for admin
        ...
        return {
            "documents": documents,
            "pagination": pagination,
            "user_role": user_role  # Include user role for frontend
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def get_user_ghostlayer_documents(request: Request, page: int = 1, limit: int = 10):
    ...
    try:
        ...
        if user_role == 'admin':
            ...
            documents = db.get_user_ghostlayer_documents(user_id=None, limit=limit, offset=offset)
            all_docs = db.get_user_ghostlayer_documents(user_id=None, limit=1000, offset=0)
        else:
        ...
        return {
            "documents": documents,
            "pagination": pagination,
            "user_role": user_role  # Include user role so frontend knows if admin
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_ghostlayer_document(request: Request, document_id: int):
    """Delete a user's GhostLayer document (admin can delete any)"""
    try:
        # Get user from session
        user_data = require_auth(request)
        if not user_data:
...
async def download_ghostlayer_document(request: Request, document_id: int):
    """Download a user's GhostLayer document (admin can download any)"""
    try:
        # Get user from session
        user_data = require_auth(request)
        if not user_data:
...
async def download_ghostlayer_coordinates(request: Request, document_id: int):
    """Download coordinates JSON file for a user's GhostLayer document (admin can download any)"""
    try:
        # Get user from session
        user_data = require_auth(request)
        if not user_data:
...
async def get_ghostlayer_document(request: Request, document_id: int):
    """Get a specific user's GhostLayer document by ID (admin can view any document)"""
    try:
        # Get user from session
        user_data = require_auth(request)
        if not user_data:
...
async def get_ghostlayer_marked_image(request: Request, document_id: int):
    """Get marked image with text coordinates using OpenCV for a user's document (admin can view any)"""
    try:
        # Get user from session
        user_data = require_auth(request)
        if not user_data:
